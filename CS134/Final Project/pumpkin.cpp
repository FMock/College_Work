...
int Pumpkin::numPumpkins = 0;

Pumpkin::Pumpkin(int x, int y, int w, int h, 
...
void Pumpkin::updateAnim(float deltaTime){
	...
	//**** Prevent Pumpkin from leaving game world **********

	if(characterPos[0] >  GC::WORLD_WIDTH - characterSize[0]){
		prev_change_x = change_x;
		change_x = 0; //stop Pumpkin from going out of bounds right
		characterPos[0] -= 1;
		moveLeft();
	}
	...
	if(characterPos[0] < 0){
		prev_change_x = change_x;
		change_x = 0; //stop Pumpkin from going out of bounds left
		characterPos[0] += 1;
		moveRight();
	}
	...
	if(characterPos[1] < 0){
		...
		change_y = 0; //stop Pumpkin from going out of bounds up
		characterPos[1] += 1;
		moveDown();
	}
...
void Pumpkin::setTime(int t){
	time = t;
}

void Pumpkin::clockTick(){
	time = timer->getElapsedTime();
}

void Pumpkin::resetTimer(){
	time = timer->getElapsedTime();
	endTime = time + behaviorChangeDuration;
	timeReset = true;
}

void Pumpkin::changeAnim(int num){
	animNum = num;
...
// Attack is moving the Pumpkin in the general direction
// of the player
void Pumpkin::attack(){
...
void Pumpkin::collision(Sprite &sprite){
	collieded = true;
...
std::string Pumpkin::to_string() const{
	ostringstream oss;
	oss << "*************************\n"
		<< "backgroundTileCollision = " << backgroundTileCollision << "\n"
		<< "collided = " << collieded << "\n" 
		<< "Sprite Type = " << type << "\n"
		<< "numPumpkins = " << Pumpkin::numPumpkins << "\n"
		<< "isDead = " << isDead << "\n"
		<< "Direction = " << direction << "\n"
		<< "isHit = "  << isHit << "\n"
		<< "AABB x = " << box.x << "\n"
